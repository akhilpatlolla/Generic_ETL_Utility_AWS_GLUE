...
args = getResolvedOptions(sys.argv, [
                          'JOB_NAME', 'URL', 'ACCOUNT', 'WAREHOUSE', 'DB', 'SCHEMA', 'USERNAME', 'PASSWORD', 'ROLE'])
...
def ETL_Utility(obj):
    for table_name in obj:
        ...
        properties = {'user': obj[table_name]['user'],
                      'password': obj[table_name]['password']}
        ...
        latestmodified = getLastUpdated_timestamp(
            target_table, timestamp_coloumn)
        print("Last Updated Time : ", latestmodified)
        Date_Condition = ''

        if latestmodified is not None:
            Date_Condition = "where {time_stamp} >= '{last_date}'".format(
                time_stamp=timestamp_coloumn, last_date=latestmodified.isoformat())
            print("Date Condition is ", Date_Condition)
            dynamic_dataFrame = Filter.apply(
                frame=dynamic_dataFrame, f=lambda x: x[timestamp_coloumn] >= latestmodified)
        ...
        snowflakeMerge(table_name, target_table, Date_Condition, snowflake_stage,
                       primary_key, list_columns, M_time, coloumns_to_delete)


obj = {
    @@template@@
    # 'source_table':{
    #     'connection_type':'mysql|redshift|oracle|sqlserver|postgresql', #JDBC connectionType values
